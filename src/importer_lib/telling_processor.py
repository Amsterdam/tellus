...
def get_tel_richting(meetlocatie_str, richting_id):
    ...
    return TelRichting.objects.get(
        richting=richting_id, tellus__meetlocatie__id=location_id
    )
...
def process_telling_sheet(file_name, csv_reader):  # noqa: C901
    ...
    for trow in csv_reader:
        ...
        for idx in range60:
            ...
            values = (
                ...
                validatie_category.id,
                meetraai_category.id,
                representatief_category.id,
            )
            batch_list[batch_idx] = values

            batch_idx += 1
            if not batch_idx % batch_insert_count:
                insert_telling_batch(batch_list)
                item_cnt += batch_insert_count

                difference = time.time() - t0  # in seconds
                log.debug(
                    f"{file_name[-10:]}, Import count: "
    ...
    if batch_idx > 0:
        ...
        insert_telling_batch(batch_list[:batch_idx])
