...
def process_tel_richting(tellus, richting, naam, zijstraat):
    tel_richting, _ = TelRichting.objects.update_or_create(
        tellus=tellus, richting=richting, naam=naam, zijstraat=zijstraat
    )
...
class TellusImporter(object):
    ...
    def process_tellus_locaties(self, title_row=1, first_col=0):
        """
        ...
        for row in self.codebook_addon_sheets["Locaties"].iter_rows(
            min_row=title_row + 1, min_col=first_col
        ):
    ...
    def process_snelheids_categorie(
        self, sheet_name="Snelheidscategorieen", title_row=1, first_col=1
    ):
        """
        ...
        for row in self.codebook_sheets[sheet_name].iter_rows(
            min_row=title_row + 1, max_row=5, min_col=first_col
        ):
            res = [
                cell.value for cell in row
            ]  # [1, '< 30 km/u', '31 - 40 km/u', ..., '> 100 km/u']
    ...
    def process_lengte_interval(
        self, sheet_name="Lengtecategorieen", title_row=1, first_col=1
    ):
        """
